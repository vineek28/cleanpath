...
# RAG retriever — lazy import so agents work even if ChromaDB isn't initialized yet
def _get_rag_context(state: str, tools: list, context_type: str = "law_mapper", law_ids: list = None) -> str:
    try:
        if context_type == "law_mapper":
            from rag.retriever import query_for_law_mapper
            return query_for_law_mapper(state, tools)
        elif context_type == "gap_scanner":
            from rag.retriever import query_for_gap_scanner
            return query_for_gap_scanner(state, tools, law_ids or [])
    except Exception as e:
        return f"[RAG unavailable: {e} — using JSON knowledge base only]"
    return ""
...
def run_law_mapper(state: dict) -> dict:
    ...
    # RAG context — real statutory text retrieved from ChromaDB
    rag_context = _get_rag_context(
        state=hospital.get("state", ""),
        tools=hospital.get("ai_tools", []),
        context_type="law_mapper"
    )
...
RAG RETRIEVED CONTEXT (from legal knowledge base — use this as primary reference):
{rag_context}

SUPPLEMENTAL REFERENCE — State laws for {state_key}:
{json.dumps(state_laws, indent=2)}

SUPPLEMENTAL REFERENCE — Federal laws that may apply:
...
def run_gap_scanner(state: dict) -> dict:
    ...
    # RAG context for gap scanning
    applicable_law_ids = [l.get("law_id", "") for l in law_output.get("applicable_laws", [])]
    rag_context = _get_rag_context(
        state=hospital.get("state", ""),
        tools=hospital.get("ai_tools", []),
        context_type="gap_scanner",
        law_ids=applicable_law_ids
    )
...
RAG RETRIEVED CONTEXT (requirement-level detail from legal knowledge base):
{rag_context}
